...
def run_gurobi_experiment(
    ...
    # Retrieve the LOGs
    if 'GUROBI_LOG_TO_CONSOLE' in os.environ:
        gurobi_logs = ''
    else:
    ...
    # Get the JSON solution
    json_solution = model.getJSONSolution()

    # Get the state of each sensor at the end
    end_sensor_state = [True]*instance.num_sensors
    for v in json.loads(json_solution).get('Vars', []):
        if v['VarName'].startswith('x'):
            v = v['VarName'].replace(']', '').split('i')[-1].split(',')[0]
            end_sensor_state[int(v)] = False
    end_sensor_state = [not e for e in end_sensor_state]
